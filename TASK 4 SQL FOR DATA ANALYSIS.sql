...
SELECT c.Customer_Id, c.Customer_Name, e.Product, e.Sales
FROM customers AS c
RIGHT JOIN `e-commerce dataset` AS e ON c.Customer_Id = e.Customer_Id;
...
CREATE INDEX idx_product_category ON `e-commerce dataset` (Product_Category(20));

-- CHECKING THE INDEXES ARE CREATED 
SHOW INDEXES FROM `e-commerce dataset`;
